...
class BatchProvider_():
    ...
    def next_batch(self, batch_size):
        ...
        real_img_idx = np.arange(self.nr_img)
        ### do arange to get img idx array from 0 to 459, then sample from this, then perform operations

        batch_img = np.random.choice(real_img_idx, size=batch_size, replace=False)
        ...
        for idx, img in zip(batch_indices, img_indices):
            batch_a[i,:,:,:] = self.data['A' + '/data_' + str(idx)][img_indices, :, :, :]
            batch_b[i,:,:,:] = self.data['B' + '/data_' + str(idx)][img_indices, :, :, :]
            i += 1
